$(function () {
    ...
    var islogin = $.cookie('Himall-User') ? "true" : "false";
    //搜索切换
    ...
    $('#searchBtn').click(function () {
        ...
        $.ajax({
            type: 'post',
            url: "/Shop/goShopByShopCode",
            dataType: 'JSON',
            data: { keyword: keyWords },
            success: function(data) {
                if (data.success) {
                    location.href = "/shop/home/" + data.ShopId + "";
                } else {
                    if (selected == "店铺") {
                        //if (keyWords == '') {
                        //    $.dialog.alert('请输入关键词！');
            ...
            success: function (data) {
                ...
                if (data.List != null) {
                    ...
                    for (var i = 0; i < data.List.length; i++) {
                        ...
                        html += '<a href="/shop/home/' + BaseShopId + '" target="_blank">';
                        html += '<h5>' + BaseShopName + '</h5>';
                        if (BaseType == 0) {
                            html += ('<h3><b>' + ApplySceneDescript + '</b>' + ManJian + '</h3>')
                            if (data.List[i].ApplySceneName != null) {
                                html += ('<div>' + data.List[i].ApplySceneName + '</div>');
                            }
                            if (data.List[i].PriceType == 0) {
                                html += '<h3><span style="bottom: 0px;">￥' + BasePrice + '</span></h3>';
                            }
                            if (data.List[i].PriceType == 1) {
                                if (parseFloat(BasePrice) == 0) {
                                    html += '<h3><span style="bottom: 0px;">' + (BasePrice == 0 ? "免费" : "可以" + BasePrice + "元购买") + '</span></h3>';
                                } else {
                                    html += '<h3><span style="bottom: 0px;">' + (parseFloat(BasePrice) / 10) + '折</span></h3>';
                                }
                            }
                            if (data.List[i].PriceType == 2 && BasePrice == 0) {
                                html += '<h3><span style="bottom: 0px;">' + (BasePrice == 0 ? "免费" : "可以" + BasePrice + "元购买") + '</span></h3>';
                            }
                            if (data.List[i].PriceType == 2 && BasePrice > 0) {
                                html += '<h3><span style="bottom: 0px;font-size:14px;">' + "" + BasePrice + "元即可购买" + '</span></h3>';
                            };
                        } else {
                            html += '<h3><span>￥' + BasePrice + '</span>' + ManJian + '</h3>';
                        }
                        html += '<p>截止' + BaseEndTime + '</p>';
                        html += '</a></li>';
                    }
...
function loadGuanZhuGoods() {
    $("#guanzhugoods").html('<li><a style=" background:url(/Images/loading2.gif) no-repeat center; display:block;  height:50px;"></a></li>');
    setTimeout(function () {
        $.ajax({
            type: 'POST',
        ...
        if (cart.products.length > 0) {
            $.each(data, function (i, e) {
                ...
                $.each(e.shop, function (j, product) {
                    ...
                    if (product.productstatus == strproductstatus) {
                        if (product.productauditstatus == strproductauditstatus) {
                            ...
                            str += '\
								<a href="/productdetail/' + product.id + '.htm" title="' + product.name + '" target="_blank"><img src="' + product.imgUrl + '" alt="" /></a>\
								<div class="s-num"><span>' + product.count + '</span></div>\
								<div class="s-g-price">'+ (product.isHidePrice?"——":(parseFloat(product.price * product.count).toFixed(2))) + '</div>\
							</div>';

                          shopPrice = shopPrice + (product.isHidePrice?0:product.price) * product.count;
                        }
                    }
                });
                list += '<li><div class="cart-list-shop cl">\
						...
						<span class="cart-shop-price" data-shoprice="'+ shopPrice.toFixed(2) + '">' + parseFloat(shopPrice).toFixed(2) + '</span>\
						</div>'+ str + '</li>';
...
function bindSelectAll() {
    $('input[name="s-checkAll"]').change(function () {
        ...
        $('#side-cart-list input[type=checkbox]').attr('checked', this.checked);
        if (checked) {
            $('#s-total-money').html(getCheckProductPrice());
            $('#side-cart-list li').each(function () {
                var shopP = $(this).find('.cart-shop-price');
                shopP.html(parseFloat(shopP.data('shoprice')).toFixed(2));
            });
        }
        else {
    ...
    $('input[name="checkShop"]').change(function () {
        ...
        if (allShopChecked) {
            ...
            $('input[name="s-checkAll"]').attr('checked', true);
        }
        else {
            $('input[name="s-checkAll"]').prop('checked', false);
            $('input[name="s-checkAll"]').attr('checked', false);
        }

        var t = 0;
        $.each($(this).parent().siblings(), function () {
            var bb = $(this).find('input[name="checkItem"]').attr("checked");
            var a = 0;
            if (bb == "checked") {
                a = $(this).find('input[name="checkItem"]').siblings('.s-g-price').html();
            }
          t += (parseFloat(a) || 0);
        })
    ...
    $('input[name="checkItem"]').change(function () {
        ...
        if (allProductChecked) {
            ...
            $(this).parent().siblings().find('input[name="checkShop"]').attr('checked', checked);
        }
        else
